...
export class SitesTableComponent implements OnInit, AfterViewInit {

  displayedColumns: string[] = ['name', 'active', 'created', 'description', 'info'];
  ...
  SITES_SEARCH: string = '/resources/sites-summary';
  ...
  search(id: string) {
    this.router.navigate([this.SITES_SEARCH, id]);
  }

  edit(id: string) {
    this.router.navigate([this.SITES_EDIT, id]);
  }
  ...
  delete(id: string) {
    this.dialogo
      ...
      .subscribe((confirmado: Boolean) => {
        ...
        if (confirmado) {
          this.siteEntityService.delete(id)
            .pipe(first())
            .subscribe({
              next: () => {
                this.openSnackBar(`Site deleted successed!!`, 'right', 'top');
