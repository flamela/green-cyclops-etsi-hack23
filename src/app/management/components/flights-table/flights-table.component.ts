...
export class FlightsTableComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ['created','lastUpdate','name', 'state', 'status', 'info'];
  ...
  FLIGHTS_SUMMARY: string = '/management/flights-summary';
  ...
  search(id: string) {
    this.router.navigate([this.FLIGHTS_SUMMARY, id]);
  }

  edit(id: string) {
    this.router.navigate([this.FLIGHTS_EDIT, id]);
  }
  ...
  delete(id: string) {
    this.dialogo
      ...
      .subscribe((confirmado: Boolean) => {
        ...
        if (confirmado) {
          this.flightEntityService.delete(id)
            .pipe(first())
            .subscribe({
              next: () => {
                this.openSnackBar(`Flight deleted successed!!`, 'right', 'top');
